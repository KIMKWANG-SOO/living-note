...
# 쓰레드(Threads) 게시 — 당분간 중단 (2026-08-15)

**게시는 하지 않는다.** 2026-08-15에 계정이 체크포인트 에러(code 190)로 게시에 실패했고, 이후 사장 확인 결과 **실제로는 쓰레드 계정이 정지된 상태**다(API 에러 문구는 "checkpoint"로 나오지만 로그인 한 번으로 풀리는 문제가 아님). 사장이 "계정 복원 전까지 제외"를 지시했다. `threads.txt` 파일은 카드뉴스 제작 시 계속 만들어두되(복원 시 바로 쓸 수 있도록), 아래 게시 절차는 **사장이 계정 복원 후 재개를 지시하기 전까지** 실행하지 않는다. 토큰 재발급을 시도하거나 권하지 말 것 — 정지 상태에서는 토큰을 새로 받아도 동일하게 막힌다.
