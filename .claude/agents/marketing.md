...
**게시는 하지 않는다.** 2026-08-15에 계정이 체크포인트 상태(code 190)에 걸려 게시가 실패했고, 사장이 "쓰레드는 당분간 빼자"고 지시했다. `threads.txt` 파일은 카드뉴스 제작 시 계속 만들어두되(재개 시 바로 쓸 수 있도록), 아래 게시 절차는 사장이 재개를 지시하기 전까지 실행하지 않는다.
