...
## 매일 발행 루틴 (자동 실행 시 이 순서를 따를 것, 3편 모두 반복)

1. **주제 선정**: `src/content/blog/`의 기존 글 목록과, 오늘 이미 발행한 편을 확인해 **중복되지 않는** 주제를 고른다. 3편은 서로 다른 카테고리로 분산한다.
...
4. **발행**: 편마다 개별로 `git add -A && git commit && git push` → Vercel이 자동 배포 (한 편씩 커밋 — 중간 실패해도 이전 편은 발행된 채로 남도록).
5. **보고**: 3편 모두 마친 뒤, 발행한 글 제목·주제와 빌드 결과를 요약해 남긴다.
